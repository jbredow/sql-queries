...
SELECT SP_DTL.ACCOUNT_NUMBER,
       ...
       SP_DTL.OG_MATRIX,
       CASE
         WHEN SP_DTL.PRICE_CATEGORY_OVR IS NOT NULL THEN SP_DTL.PR_OVR
         ELSE NULL
       END
         PR_OVR,
       CASE
         WHEN SP_DTL.PRICE_CATEGORY_OVR IS NOT NULL THEN SP_DTL.PR_OVR_BASIS
         ELSE NULL
       END
         PR_OVR_BASIS,
       CASE
         WHEN SP_DTL.PRICE_CATEGORY_OVR IS NOT NULL THEN SP_DTL.GR_OVR
         ELSE NULL
       END
         GR_OVR,
       SP_DTL.TRIM_FORM,
  ...
  FROM ( SELECT SP_HIST.*,
           ...
           FROM           ( SELECT IHF.ACCOUNT_NUMBER,
                             ...
                             WHERE IHF.INVOICE_NUMBER_GK =
                                   ...
                                   --AND IHF.ACCOUNT_NUMBER IN ( '215', '230', '564', '88', '216' )
                                   -- AND NVL (ILF.PRICE_CODE, 'N/A') IN
                                   --       ('Q', 'N/A', 'R')
                                   /*AND IHF.SALESREP_GK IN
                                          ( '27481799',
                                           '7712508',
                                           '30723352',
                                           '31008887',
                                   ...
                                   -- AND IHF.WAREHOUSE_NUMBER IN ('2716', '2725')
                    ...
                    ( SELECT COD.BASIS,
                       ...
                       WHERE COD.OVERRIDE_TYPE = 'G'
                             ...
                             AND NVL ( COD.EXPIRE_DATE, SYSDATE ) >=
                                  (SYSDATE - 395) ) GR_OVR
                  ON ( SP_HIST.DISCOUNT_GROUP_NK = (LTRIM ( GR_OVR.DISC_GROUP,
                                                           '0'
                                                    ))
                ...
                LEFT OUTER JOIN
                  ( SELECT COD.BASIS,
                           ...
                           END
                             FORMULA
                      FROM DW_FEI.CUSTOMER_OVERRIDE_DIMENSION COD
                     WHERE COD.OVERRIDE_TYPE = 'P' AND COD.DELETE_DATE IS NULL
                           AND NVL ( COD.EXPIRE_DATE, SYSDATE ) >=
                                (SYSDATE - 395) ) PR_OVR
