SELECT SWD.DIVISION_NAME
       ...
       END
          SHOW_PROD_GRP,
       CASE
          WHEN DTL.DISCOUNT_GROUP_NK IN ('2229',
                                         '2236',
                                         '2243',
                                         '2257',
                                         '2258',
       ...
       /*SUM (
          ...
          END)
          AS OLD_AVG_COGS,*/
       SUM (DTL.CORE_ADJ_AVG_COST)
          AS AVG_COGS,
       SUM (DTL.EXT_SALES_AMOUNT)
          AS EXT_SALES,
       CASE
          WHEN DTL.PRICE_CATEGORY IN ('OTH/ERROR', 'QUOTE', 'TOOLS')
          THEN
             'MANUAL'
          WHEN DTL.PRICE_CATEGORY IN ('MATRIX_BID', 'NDP')
          THEN
             'MATRIX'
          ELSE
             DTL.PRICE_CATEGORY
...
FROM [DWFEI_STG].[PR_VICT2_CUST_R2MO] DTL
     ...
     INNER JOIN [DWFEI_STG].[BG_CUSTTYPE_XREF] BG
...
--LEFT OUTER JOIN PRICE_MGMT.PR_SHOW_PILOT S
--  ON DTL.DISCOUNT_GROUP_NK = S.DG
WHERE     DTL.ACCOUNT_NAME IN ('PLYMOUTH',
                               'PHOENIX',
                               'ORL',
                               'LENZ',
                               'LYNN')
      AND DTL.YEARMONTH >= 201904
--AND DTL.PROCESS_DATE > (SYSDATE - 14)
