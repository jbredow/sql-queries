SELECT p_cat.YEARMONTH,
       ...
       wh.REGION_NAME,
	   wh.DISTRICT,
       SUBSTRING (wh.DISTRICT, 1, 3) DIST,
       wh.ACCOUNT_NAME,
       h.HILEV,
       h.DET1,
       h.DET2,
       h.DET6,
	   CONCAT(h.DISCOUNT_GROUP_NK, ' - ', h.DESCRIPTION) DG_NUM_NAME,
       h.DISCOUNT_GROUP_NK,
       h.DESCRIPTION,
       CASE
...
FROM ((([DWFEI_STG].[TIME_PERIOD_DIMENSION] tpd
	   ...
	   INNER JOIN [DWFEI_STG].[WHSE_RPT_AREA_VW] wh
		  ON (
			CONVERT(VARCHAR(8),p_cat.WAREHOUSE_NUMBER) = CONVERT(VARCHAR(8),wh.WAREHOUSE_NUMBER_NK)))
