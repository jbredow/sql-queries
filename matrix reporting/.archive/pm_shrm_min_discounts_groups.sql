/* 		uses the DW_FEI.PRICE_DIMENSION PRICE table
...
FROM(
	...
	FROM (
		SELECT PRICE.BRANCH_NUMBER_NK Branch_Num,
			...
			CASE      
				...
				WHEN (PRICE.BASIS = 'L' AND PRICE.OPERATOR_USED = '-') THEN ROUND((PRICE.MULTIPLIER),3)
				WHEN (PRICE.BASIS = 'L' AND PRICE.OPERATOR_USED = 'X') THEN ROUND(1-PRICE.MULTIPLIER,3)
				WHEN (PRICE.BASIS = 'L' AND PRICE.OPERATOR_USED = '+') THEN ROUND(1-(1+PRICE.MULTIPLIER),3)		
