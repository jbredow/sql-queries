...
SELECT
	...
	V2.YEARMONTH,
	V2.PRICE_COLUMN,
	PCD.PRICE_COLUMN_NAME,
	V2.TYPE_OF_SALE,
	V2.PRICE_CATEGORY,
	SUM ( V2.EXT_SALES_AMOUNT ) SALES,
	SUM ( V2.EXT_AVG_COGS_AMOUNT ) AC,
	SUM( ( V2.EXT_SALES_AMOUNT - V2.EXT_AVG_COGS_AMOUNT ) / V2.EXT_SALES_AMOUNT ) GP_PCT
	FROM (
		SELECT DISTINCT
			sp_dtl.YEARMONTH,
			...
			NVL (sp_dtl.PRICE_CATEGORY_OVR, sp_dtl.PRICE_CATEGORY) PRICE_CATEGORY,
			sp_dtl.PRICE_COLUMN
		...
		FROM
			...
			FROM (SELECT IHF.ACCOUNT_NUMBER,
					...
					WHERE IHF.INVOICE_NUMBER_GK = ILF.INVOICE_NUMBER_GK 
						...
						--AND CUST.MAIN_CUSTOMER_NK IN('879', '53748', '53891')
				...
				LEFT OUTER JOIN (SELECT COD.BASIS,
					 ...
					 ON (	 SP_HIST.PRODUCT_NK = PR_OVR.MASTER_PRODUCT
						 ...
						 AND NVL(SP_HIST.CONTRACT_NUMBER,'DEFAULT_MATCH') = 
								NVL(PR_OVR.CONTRACT_ID, 'DEFAULT_MATCH'))) sp_dtl
			) v2
			LEFT OUTER JOIN AAD9606.PRICE_COLUMN_DEFINITION PCD
				ON (PCD.PRICE_COLUMN = V2.PRICE_COLUMN)
GROUP BY V2.YEARMONTH, V2.PRICE_COLUMN, PCD.PRICE_COLUMN_NAME, V2.TYPE_OF_SALE, V2.PRICE_CATEGORY;