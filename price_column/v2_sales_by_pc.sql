	...
	FROM (
	  ...
	  FROM    (SELECT SP_HIST.*,
		...
		LEFT OUTER JOIN (SELECT DISTINCT 
							TO_CHAR (PC.PRICE_COLUMN, '000') PC,
							PC.COLUMN_NAME
							FROM AAD9606.PR_COLUMN_STRATEGY_2014 PC
							WHERE PC.STATUS = 'Valid'
						) PRCOL
						ON	PRCOL.PC = V2.PRICE_COLUMN

	
	WHERE V2.ACCOUNT_NUMBER IN ('190')
    --AND V2.CHANNEL_TYPE IN ('Showroom', 'Showroom Direct')


GROUP BY V2.TYPE_OF_SALE, V2.PRICE_COLUMN, PRCOL.COLUMN_NAME, CASE
		WHEN V2.price_category_ovr IS NOT NULL
		THEN V2.price_category_ovr
		ELSE V2.price_category
	END;