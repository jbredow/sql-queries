/* 
	use for monthly whse channel usage report 
	AAE0376.GP_TRACKER_13MO GP_DATA  jenn's orr
	change line 655 to proper schema
*/
...
SELECT GP_DATA.YEARMONTH,
       ...
       SUM (
          ...
          "Subtotal GP$"
  --GP_DATA.TYPE_OF_SALE
  FROM AAE0376.GP_TRACKER_13MO GP_DATA,  --Change to the proper schema if necessary
       (SELECT WD.ACCOUNT_NAME, WD.ACCOUNT_NUMBER_NK
          FROM DW_FEI.WAREHOUSE_DIMENSION WD
         WHERE     (WD.ACTIVE_FLAG = 1)
               AND (WD.DELETE_DATE IS NULL)
               AND WD.ACCOUNT_NAME <> 'FRESNO'
 ...
 WHERE GP_DATA.ACCOUNT_NUMBER = ACCT.ACCOUNT_NUMBER_NK(+)
       ...
       /* AND GP_DATA.YEARMONTH = (SELECT MAX (GP_DATA.YEARMONTH)
                                  FROM AAE0376.GP_TRACKER_12MO GP_DATA) */
       AND GP_DATA.TYPE_OF_SALE <> 'Credit Memo'
