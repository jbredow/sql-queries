...
FROM (SELECT NEW_CUST.CUSTOMER_NK "Cust #",
             ...
             NEW_CUST.SEC_SLSM "Secondary SalesRep"
      /* NEW_CUST.CUSTOMER_GK CUST_GK,
         ...
         */
      FROM (SELECT --NVL (PS_HIERARCHY.KIND_OF_BUSINESS, N'OTHER')
                   --BRANCH_KOB,
                   ...
                   CUST.MSTR_CUST_NAME,
                   CUST.MSTR_TYPE,
                   BG.BUSINESS_GROUP BUS_GRP,
                   CUST.JOB_YN,
                   CASE
                      WHEN LTRIM (CUST.SALESMAN_CODE, '0123456789') IS NULL
                      THEN
                         'Y'
                   ...
                   CASE
                      ...
                      THEN
                         CASE
                            ...
                            WHEN CUST.PRICE_COLUMN IN ('204', '205') THEN 'Y'
                            ELSE 'N'
                         END
                      ELSE
                         'N'
                   ...
                   CASE
                      ...
                      THEN
                         CASE
                            WHEN PC_CT_XREF.CATEGORY IN 'Fire Protection'
                            THEN
                               'Y'
                            ELSE
                               'N'
                   ...
                   END
                      KOB_ALIGNED,
                   NVL (PC_DEF.COLUMN_NAME, 'INVALID')
                      COLUMN_DEFINITION,
                   NVL (PC_DEF.CATEGORY, 'INVALID')
                      COLUMN_CATEGORY,
            ...
            FROM DW_FEI.CUSTOMER_DIMENSION CUST
                 LEFT OUTER JOIN SCORECARD1.PS_HIERARCHY PS_HIERARCHY
                    ON TO_CHAR (CUST.BRANCH_WAREHOUSE_NUMBER) =
                       TO_CHAR (
                          LTRIM (PS_HIERARCHY.WAREHOUSE_NUMBER_NK, '0'))
                 LEFT OUTER JOIN
                 ...
                 LEFT OUTER JOIN PRICE_MGMT.PR_CTYPE_COLUMN_XREF PC_CT_XREF
                    ON (    (CUST.CUSTOMER_TYPE = PC_CT_XREF.CUSTOMER_TYPE)
                        AND TO_CHAR (LTRIM (CUST.PRICE_COLUMN, '0')) =
                            TO_CHAR (LTRIM (PC_CT_XREF.PC, '0')))
                 LEFT OUTER JOIN USER_SHARED.BG_CUSTTYPE_XREF BG
                    ON BG.CUSTOMER_TYPE = CUST.CUSTOMER_TYPE
                 LEFT OUTER JOIN
                 (SELECT DISTINCT PC PRICE_COLUMN,
                                  PC_NAME COLUMN_NAME,
                                  BUSINESS_GROUP CATEGORY,
                                  CATEGORY SUB_CATEGORY
                  FROM PRICE_MGMT.PR_CTYPE_COLUMN_XREF) PC_DEF
                 ...
                 (SELECT DISTINCT
                         ...
                         SUB_CATEGORY,
                         CUSTOMER_TYPE,
                         CUSTOMER_GROUP || ' - ' || SUB_CATEGORY
                            CTYPE_DEFINITION
                  FROM USER_SHARED.BG_CUSTTYPE_XREF) CTYPE1
            ...
            WHERE     (CUST.DELETE_DATE IS NULL)
                  ...
                  --       TRUNC (SYSDATE - 183)
                  AND TRUNC (
                         NVL (CUST.ACCOUNT_SETUP_DATE, CUST.INSERT_TIMESTAMP)) BETWEEN TRUNC (
                                                                                            SYSDATE
                                                                                          - 9)
                                                                                   AND TRUNC (
                                                                                            SYSDATE
      ...
      WHERE NEW_CUST.SEC_SLSM IN ('XXX')
        AND NEW_CUST.ACCOUNT_NAME = 'OHVAL'
      --AND NEW_CUST.HOUSE_ACCT = 'N'
