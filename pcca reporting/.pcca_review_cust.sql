SELECT DISTINCT CUST.ACCOUNT_NUMBER_NK BRANCH_NK,
       CUST.ACCOUNT_NAME BRANCH_NAME,
       CUST.MAIN_CUSTOMER_NK MAIN_NK,
       ...
       CUST.SALESMAN_CODE SLSM,
			 CUST.CROSS_CUSTOMER_NK "X-CUST"
			 
  FROM   DW_FEI.CUSTOMER_DIMENSION CUST
       INNER JOIN
         SALES_MART.SALES_WAREHOUSE_DIM SWD
       ON ( SWD.ACCOUNT_NUMBER_NK = CUST.ACCOUNT_NUMBER_NK )
 WHERE ( SUBSTR ( SWD.REGION_NAME,
       ...
       AND CUST.ACCOUNT_NUMBER_NK = '226'
			 ...
			 AND CUST.CUSTOMER_NAME <> 'UNAVAILABLE'
			 
			 RIGHT OUTER JOIN (
			 		SELECT
									X.ACCOUNT_NUMBER_NK BRANCH_NK,
									NVL ( X.SELL_ALIAS_NAME, NULL ) BRANCH_NAME, 
	...
	ON CUST.ACCOUNT_NUMBER_NK = SALES.BRANCH_NK
		AND CUST.MAIN_CUSTOMER_NK  = SALES.MAIN_NO
		AND  CUST.CUSTOMER_NK = SALES.CUST_NO
