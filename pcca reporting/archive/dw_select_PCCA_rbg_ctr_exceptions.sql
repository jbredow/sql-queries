SELECT DISTINCT *
  FROM (SELECT NEW_CUST.CUSTOMER_GK CUST_GK,
               ...
               NEW_CUST.RBG_COLUMN_ALIGNED RBG_ALIGN,
               NEW_CUST.LOW_SPEND,
          ...
          FROM (SELECT DISTINCT
                       ...
                       CASE
                          ...
                          THEN
                             'N'
                          WHEN TO_NUMBER (LTRIM (CUST.PRICE_COLUMN, '0')) BETWEEN (PC_CT_XREF.
                                                                                   MIN_COLUMN)
                                                                              AND (PC_CT_XREF.
                                                                                   MAX_COLUMN)
                          THEN
                       ...
                       END
                          AS COLUMN_ALIGNED,
                       CASE
                          WHEN CUST.PRICE_COLUMN IN
                                  ('001', '002', '003', '004')
                          THEN
                             CASE
                                WHEN CUST.CUSTOMER_TYPE NOT IN
                                        ('C_PLBGHVAC',
                                         'P_PLBGCON',
                                         'P_PLBGCON_R&R')
                                THEN
                       ...
                       END
                          ...
                          AS RBG_COLUMN_ALIGNED,
                       CASE
                          WHEN CUST.PRICE_COLUMN IN
                                  ('006', '007', '008', '009')
                          THEN
                             CASE
                                WHEN NVL (BG_CUBE.ROLL_12M_SLS, 0) < 12000
                                THEN
                                   'Y'
                                ELSE
                                   'N'
                 ...
                 WHERE (CUST.DELETE_DATE IS NULL)
                       AND LTRIM (CUST.PRICE_COLUMN, '0') IN
                              ('1', '2', '3', '4', '5', '6', '7', '8', '9')
         ...
         WHERE --(NEW_CUST.RBG_COLUMN_ALIGNED = 'N' OR NEW_CUST.LOW_SPEND = 'Y')
         (NVL(NEW_CUST.RBG_COLUMN_ALIGNED,'Y') || NEW_CUST.LOW_SPEND) IN ('NY', 'YY', 'NN')
         AND NEW_CUST.HOUSE_ACCT='N'
         AND NEW_CUST.JOB_YN='N'
         AND NEW_CUST.BRANCH_KOB IN ('PLB','PLBG')
		 --
		 AND NEW_CUST.ACCOUNT_NAME = "DALLAS"
		 --
