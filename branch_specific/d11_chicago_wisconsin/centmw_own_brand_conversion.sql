...
SELECT DISTINCT
       ...
       sp_dtl.ACCOUNT_NAME,*/
       sp_dtl.WAREHOUSE_NUMBER WHSE_NK,
       --sp_Dtl.CHANNEL_TYPE,
       --sp_dtl.INVOICE_LINE_NUMBER,
       --sp_dtl.MANUFACTURER,
       --sp_dtl.PRODUCT_NK,
       CASE
        WHEN sp_Dtl.ALT1_CODE IN ('K2210-0',
                                  'K2210-96',
                                  'K2882-0',
                                  'K2214-0',
                                  'K2214-96',
          ...
          'Kohler Undermount Lavs'
        WHEN sp_Dtl.ALT1_CODE IN ('K4636-0',
                                  'B1500EC000',
                                  'K4774-0',
                                  'K4639-0',
                                  'K4636-96',
       ...
       sp_dtl.STATUS,
       sp_dtl.ORDERED_QTY ORDERED,
       --sp_dtl.SHIPPED_QTY,
       sp_dtl.UNIT_NET_PRICE_AMOUNT NET,
       (sp_dtl.UNIT_NET_PRICE_AMOUNT * sp_dtl.ORDERED_QTY) EXT_TOTAL,
       --(sp_dtl.CORE_ADJ_AVG_COST * sp_dtl.ORDERED_QTY) CORE_AVG_COST_TOTAL,
...
FROM (SELECT SP_HIST.*, --process date changed to include invoice processing date
      ...
      FROM (SELECT IHF.ACCOUNT_NUMBER,
                   ...
                   CASE
                      ...
                      THEN
                         'MANUAL'
                      WHEN NVL (ilf.orig_price_code, ilf.price_code) = '*E'
                      THEN
                         'OTH/ERROR'
                      WHEN NVL (ilf.orig_price_code, ilf.price_code) = 'SKC'
                      THEN
                         'OTH/ERROR'
                      WHEN NVL (ilf.orig_price_code, ilf.price_code) IN
                              ('%',
                      ...
                      THEN
                         'TOOLS'
                      WHEN NVL (ilf.orig_price_code, ilf.price_code) IS NULL
                      THEN
                         'MANUAL'
                      ELSE
                         'MANUAL'
            ...
            WHERE     IHF.INVOICE_NUMBER_GK = ILF.INVOICE_NUMBER_GK
                          ...
                          AND ILF.EXT_SALES_AMOUNT = 0
                          AND ILF.ORDERED_QTY > 0
           ...
           GR_OVR_BASE
              ON (    SP_HIST.DISCOUNT_GROUP_NK =
                  ...
                  AND SP_HIST.ACCOUNT_NUMBER = GR_OVR_BASE.BRANCH_NUMBER_NK
      ...
      ORDER BY PRODUCT_CATEGORY, ALT1_CODE, WAREHOUSE_NUMBER, WRITER
