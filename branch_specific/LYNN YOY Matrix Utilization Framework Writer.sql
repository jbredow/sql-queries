SELECT SLS.REGION,
       ...
       NVL (
          SUM (
             CASE
                WHEN SLS.ROLL12MONTHS IN 'YEAR TO DATE'
                THEN
       ...
       SUM (
          CASE
             WHEN     SLS.PRICE_CATEGORY_FINAL IN
                  ...
                  AND SLS.ROLL12MONTHS IN 'YEAR TO DATE'
             THEN
                SLS.EXT_SALES_AMOUNT
             ELSE
                0
       ...
       NVL (
          SUM (
             CASE
                WHEN SLS.ROLL12MONTHS IN 'LAST YEAR TO DATE'
                THEN
                   SLS.EXT_SALES_AMOUNT
                ELSE
                   0
       ...
       SUM (
          CASE
             WHEN     SLS.PRICE_CATEGORY_FINAL IN
                         ('MATRIX', 'MATRIX_BID', 'NDP')
                  AND SLS.ROLL12MONTHS IN 'LAST YEAR TO DATE'
             THEN
                SLS.EXT_SALES_AMOUNT
             ELSE
                0
...
FROM (SELECT TPD.ROLL12MONTHS,
      ...
      WHERE     TPD.FISCAL_YEAR_TO_DATE IS NOT NULL
            ...
            --   AND ILF.PRODUCT_NK IN ('3916528', '7345096')
