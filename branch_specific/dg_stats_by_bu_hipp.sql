...
FROM (  SELECT DISTINCT
        ...
        GROUP BY SWD.REGION_NAME,
                ...
                PRICE.DISC_GROUP,
                NAT_PC.PRICE_COLUMN
      ) STATS  -- 0:1:03 duration @ 2000
			
    LEFT OUTER JOIN
         ( SELECT  COD.BRANCH_NUMBER_NK,
										NVL (PROD.DISCOUNT_GROUP_NK, COD.DISC_GROUP) DISC_GROUP,
										SUM (CASE WHEN COD.OVERRIDE_TYPE = 'P' THEN 1 ELSE 0 END) PROD_CCOR,
										SUM (CASE WHEN COD.OVERRIDE_TYPE = 'G' THEN 1 ELSE 0 END) GROUP_CCOR,
										SUM (CASE WHEN COD.OVERRIDE_TYPE = 'C' THEN 1 ELSE 0 END) COST_CCOR
							...
							GROUP BY COD.BRANCH_NUMBER_NK, NVL (PROD.DISCOUNT_GROUP_NK, COD.DISC_GROUP)
        ) CCOR -- 0:1:23 duration @ 2000
				
      ON  	 TO_NUMBER ( CCOR.BRANCH_NUMBER_NK ) = TO_NUMBER ( STATS.ACCOUNT_NUMBER_NK )
        AND  TO_NUMBER ( CCOR.DISC_GROUP ) = TO_NUMBER ( STATS.DISC_GROUP )
    ...
    LEFT OUTER JOIN
        ...
        ) DPRO  -- 0:01:08 duration @ 2000
				
      ON     TO_NUMBER ( DPRO.ACCOUNT_NUMBER_NK ) = TO_NUMBER ( STATS.ACCOUNT_NUMBER_NK )
				AND  TO_NUMBER ( DPRO.DISCOUNT_GROUP_NK ) = TO_NUMBER ( CCOR.DISC_GROUP )
    ...
    LEFT OUTER JOIN
          ( SELECT  PM_DET.ACCOUNT_NUMBER_NK,
										...
										MAX (
												CASE
													WHEN PM_DET.PRICE_CATEGORY IN ('MATRIX', 'MATRIX_BID')
													THEN
															PM_DET.YEARMONTH
													ELSE
															NULL
							...
							WHERE     (TPD.ROLL12MONTHS = 'LAST TWELVE MONTHS')
										...
										AND SUBSTR (PM_DET.SELL_DISTRICT, 1, 3) IN ('D10',
																																'D11',
																																'D12',
																																'D14',
																																'D30',
      ...
      ON    TO_NUMBER ( SALES.ACCOUNT_NUMBER_NK ) = TO_NUMBER ( STATS.ACCOUNT_NUMBER_NK )
        AND TO_NUMBER ( SALES.DISCOUNT_GROUP_NK ) = TO_NUMBER ( STATS.DISC_GROUP )
		...
		LEFT OUTER JOIN
					(SELECT DISTINCT
						...
						WHERE     ( PRICE.DELETE_DATE IS NULL )
									...
									-- AND ( PRICE.DISC_GROUP = '3915' )
						GROUP BY PRICE.BRANCH_NUMBER_NK, PRICE.DISC_GROUP, PRICE.DELETE_DATE ) DATES
						
				ON    TO_NUMBER ( DATES.BRANCH_NUMBER_NK ) = TO_NUMBER ( SALES.ACCOUNT_NUMBER_NK )
					AND TO_NUMBER ( DATES.DISC_GROUP ) = TO_NUMBER ( SALES.DISCOUNT_GROUP_NK )
