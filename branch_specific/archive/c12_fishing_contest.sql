...
SELECT 
		...
		XX.WAREHOUSE_NUMBER,
		--XX.TYPE_OF_SALE,
		SUM ( XX.EXT_SALES_AMOUNT ) EX_SALES,
		SUM ( XX.EXT_AVG_COGS_AMOUNT ) EX_AC,
		SUM (
			CASE WHEN PRICE_CAT IN ( 'MATRIX', 'MATRIX_BID' )
				THEN XX.EXT_SALES_AMOUNT
				ELSE 0
			END
			) MATRIX_SALES,
		SUM (
			CASE WHEN PRICE_CAT IN ( 'MATRIX', 'MATRIX_BID' )
				THEN XX.EXT_AVG_COGS_AMOUNT
				ELSE 0
			END
			) MATRIX_AC,
		SUM (
			CASE WHEN PRICE_CAT = 'OVERRIDE'
				THEN XX.EXT_SALES_AMOUNT
				ELSE 0
			END
			) OVERRIDE_SALES,
		...
		SUM (
			CASE WHEN PRICE_CAT IN ( 'MANUAL', 'TOOLS', 'OTH/ERROR', 'QUOTE' )
				THEN XX.EXT_SALES_AMOUNT
				ELSE 0
			END
			) MANUAL_SALES,
		...
		FROM    (SELECT SP_HIST.*,
							...
							FROM (SELECT IHF.ACCOUNT_NUMBER,
											...
											WHERE     IHF.INVOICE_NUMBER_GK = ILF.INVOICE_NUMBER_GK 
														...
														--AND ILF.YEARMONTH = '201502'
														--AND IHF.YEARMONTH = '201502'
														AND ILF.YEARMONTH BETWEEN '201502' AND '201507'
														AND IHF.YEARMONTH BETWEEN '201502' AND '201507'
