-- oatey contest - final version
-- select dates includes aug-sep 14 and pulls up to -1 day
...
SELECT DISTINCT
		...
		sp_dtl.TYPE_OF_SALE,  --(e.g. counter)
		...
		-- sp_Dtl.CHANNEL_TYPE,
		NVL(SP_DTL.SALESMAN_CODE, NULL) SLSM,
		...
		SUM ( sp_dtl.EXT_SALES_AMOUNT ) SALES,
		SUM ( sp_dtl.EXT_AVG_COGS_AMOUNT ) AC
  ...
  FROM    (SELECT SP_HIST.*,
		 ...
		 FROM (SELECT IHF.ACCOUNT_NUMBER,
					  ...
					  NVL(CUST.SALESMAN_CODE, NULL) SALESMAN_CODE,
					  IHF.CONTRACT_NUMBER,
					  IHF.OML_ASSOC_NAME,
				...
				WHERE     IHF.INVOICE_NUMBER_GK = ILF.INVOICE_NUMBER_GK 
					  ...
					  AND ILF.YEARMONTH IN ( '201408', '201409')
					  AND IHF.YEARMONTH IN ( '201408', '201409')
					  ) SP_HIST
	...
	WHERE sp_dtl.ACCOUNT_NUMBER IN ( '448', '520', '1550', '1674', '3093', '1020', '226', '1657' ) 
		AND sp_dtl.DISCOUNT_GROUP_NK IN ( 1521,
											1523,
											1530,
											1531,
											1534,
											...
											2304,
											8665 )
		AND (TRUNC (sp_dtl.ORDER_ENTRY_DATE) BETWEEN TRUNC (
																SYSDATE - 90)
											 AND TRUNC (
																SYSDATE - 1))
