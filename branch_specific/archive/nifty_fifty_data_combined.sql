/* 
	by BU by DG by PC
	culls previous month data - wait for data to update in SALES_MART.TIME_DIM 
	before running query > else change yearmonth statement
*/
...
SELECT XX.YEARMONTH,
       ...
       CASE
          ...
          ELSE
             CASE
                WHEN XX.PRICE_CATEGORY = 'MATRIX_BID' THEN 'MATRIX'
                ELSE XX.PRICE_CATEGORY
             END
       END
          PRICE_CAT,
       SUM (XX.EXT_SALES_AMOUNT) EXT_SALES,
       SUM (XX.EXT_AVG_COGS_AMOUNT) EXT_AVG_COST
  FROM (SELECT DISTINCT
          ...
          FROM (SELECT SP_HIST.*,
                       ...
                       NVL (PR_OVR.INSERT_TIMESTAMP, GR_OVR.INSERT_TIMESTAMP)
                          CCOR_CREATE,
                       NVL (PR_OVR.EXPIRE_DATE, GR_OVR.EXPIRE_DATE)
                          CCOR_EXPIRE,
                       LB.LINEBUY_NAME,
                  ...
                  FROM                (SELECT IHF.ACCOUNT_NUMBER,
                                        ...
                                        WHERE IHF.INVOICE_NUMBER_GK =
                                              ...
                                              --AND CUST.CUSTOMER_NK = '2237'
                                              --AND PROD.LINEBUY_NK='200'
                                              AND IHF.CUSTOMER_ACCOUNT_GK =
                                                    CUST.CUSTOMER_GK
                                              AND DECODE (NVL (cust.ar_gl_number,
                                                               '9999'
                                                          ),
                                                          '1320',
                                                          0,
                       ...
                       ON (SP_HIST.PRODUCT_NK = PR_OVR.MASTER_PRODUCT
                           AND SP_HIST.ACCOUNT_NUMBER =
                                 PR_OVR.BRANCH_NUMBER_NK
                           AND SP_HIST.CUSTOMER_ACCOUNT_GK =
                                 PR_OVR.CUSTOMER_GK)) sp_dtl) XX
GROUP BY XX.YEARMONTH,
         XX.ACCOUNT_NUMBER,
         XX.DISCOUNT_GROUP_NK,
         XX.PRICE_COLUMN,
         XX.ALT1_CODE,
