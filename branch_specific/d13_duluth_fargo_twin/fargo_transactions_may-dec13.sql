/* 
	builds transaction data and culls DG info from that 
*/


SELECT 	TRANS.DISCOUNT_GROUP_NK,
		TRANS.DISCOUNT_GROUP_NAME,
		TRANS.CHANNEL_TYPE,
		TRANS.TYPE_OF_SALE,		
		TRANS.SHIP_VIA_NAME,
		TRANS.YEARMONTH,
		TRANS.PRICE_CODE,
		TRANS.PRICE_CATEGORY,
		TRANS.PRICE_CATEGORY_OVR,
		TRANS.ORDER_CODE,
		TRANS.SOURCE_SYSTEM,

		SUM(TRANS.SHIPPED_QTY) SHIPPED,
		SUM(TRANS.EXT_SALES_AMOUNT) SALES,
		SUM(TRANS.EXT_AVG_COGS_AMOUNT)AC

FROM (
	SELECT DISTINCT
		TRANSACTIONS.ACCOUNT_NUMBER,
		TRANSACTIONS.ACCOUNT_NAME,
		TRANSACTIONS.WAREHOUSE_NUMBER,
		TRANSACTIONS.INVOICE_NUMBER_NK,
		TRANSACTIONS.TYPE_OF_SALE,
		TRANSACTIONS.SHIP_VIA_NAME,
		TRANSACTIONS.OML_ASSOC_INI,
		TRANSACTIONS.OML_FL_INI,
		TRANSACTIONS.OML_ASSOC_NAME,
		TRANSACTIONS.WRITER,
		TRANSACTIONS.WR_FL_INI,
		TRANSACTIONS.DISCOUNT_GROUP_NK,
		TRANSACTIONS.DISCOUNT_GROUP_NAME,
		TRANSACTIONS.CHANNEL_TYPE,
		--NVL (TRANSACTIONS.ASSOC_NAME, emp.ASSOC_NAME) ASSOC_NAME,
		TRANSACTIONS.INVOICE_LINE_NUMBER,
		TRANSACTIONS.MANUFACTURER,
		TRANSACTIONS.PRODUCT_NK,
		TRANSACTIONS.ALT1_CODE,
		TRANSACTIONS.PRODUCT_NAME,
		TRANSACTIONS.STATUS,
		TRANSACTIONS.SHIPPED_QTY,
		TRANSACTIONS.EXT_SALES_AMOUNT,
		TRANSACTIONS.EXT_AVG_COGS_AMOUNT,
		TRANSACTIONS.REPLACEMENT_COST,
		TRANSACTIONS.UNIT_INV_COST,
		TRANSACTIONS.PRICE_CODE,
		TRANSACTIONS.PRICE_CATEGORY,
		TRANSACTIONS.PRICE_CATEGORY_OVR,
		TRANSACTIONS.PRICE_FORMULA,
		TRANSACTIONS.UNIT_NET_PRICE_AMOUNT,
		TRANSACTIONS.UM,
		TRANSACTIONS.SELL_MULT,
		TRANSACTIONS.PACK_QTY,
		TRANSACTIONS.LIST_PRICE,
		TRANSACTIONS.MATRIX_PRICE,
		TRANSACTIONS.MATRIX,
		TRANSACTIONS.OG_MATRIX,
		CASE WHEN TRANSACTIONS.PRICE_CATEGORY_OVR IS NOT NULL THEN 
		TRANSACTIONS.PR_OVR ELSE NULL END PR_OVR,
		CASE WHEN TRANSACTIONS.PRICE_CATEGORY_OVR IS NOT NULL THEN 
		TRANSACTIONS.PR_OVR_BASIS ELSE NULL END PR_OVR_BASIS,
		CASE WHEN TRANSACTIONS.PRICE_CATEGORY_OVR IS NOT NULL THEN 
		TRANSACTIONS.GR_OVR ELSE NULL END GR_OVR,
		TRANSACTIONS.TRIM_FORM,
		TRANSACTIONS.ORDER_CODE,
		TRANSACTIONS.SOURCE_SYSTEM,
		TRANSACTIONS.CONSIGN_TYPE,
		TRANSACTIONS.MAIN_CUSTOMER_NK,
		TRANSACTIONS.CUSTOMER_NK,
		TRANSACTIONS.CUSTOMER_NAME,
		TRANSACTIONS.PRICE_COLUMN,
		TRANSACTIONS.CUSTOMER_TYPE,
		TRANSACTIONS.REF_BID_NUMBER,
		TRANSACTIONS.SOURCE_ORDER,
		TRANSACTIONS.ORDER_ENTRY_DATE,
		TRANSACTIONS.COPY_SOURCE_HIST,
	...
	FROM    (SELECT SP_HIST.*,
             ...
             FROM (SELECT IHF.ACCOUNT_NUMBER,
                    ...
                    WHERE     IHF.INVOICE_NUMBER_GK = ILF.INVOICE_NUMBER_GK 
                          ...
                          --AND IHF.ACCOUNT_NUMBER = '1500'
                          --AND NVL (ILF.PRICE_CODE, 'N/A') IN
                          --      ('Q', 'N/A', 'R')
                          --AND IHF.WRITER = 'CMC'
                          AND CUST.ACCOUNT_NAME IN ('FARGO')
                  ...
                  LEFT OUTER JOIN (SELECT COD.BASIS,
                     ...
                     ON (    SP_HIST.PRODUCT_NK = PR_OVR.MASTER_PRODUCT
                         ...
                         AND SP_HIST.CUSTOMER_ACCOUNT_GK = PR_OVR.CUSTOMER_GK)) TRANSACTIONS) trans
