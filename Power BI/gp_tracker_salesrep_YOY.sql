...
ROUND (
   ...
   3)
   SP_UTIL,*/
FROM (SELECT SLS.SELL_REGION_NAME
                REGION,
             SLS.SELL_DISTRICT
                DISTRICT,
             ...
             TPD.FISCAL_YEAR_TO_DATE
                FYTD,
             NVL (SUM (SLS.EXT_SALES_AMOUNT), 0)
                EX_SALES,
             NVL (SUM (SLS.EXT_AVG_COGS_AMOUNT), 0)
                EX_COGS,
             /* MATRIX */
             SUM (
                CASE
             ...
             SUM (
                CASE
                   WHEN NVL (PRICE_SUB_CATEGORY, PRICE_CATEGORY) = 'OVERRIDE'
                   THEN
                      SLS.EXT_SALES_AMOUNT
                   ELSE
                      0
             ...
             SUM (
                CASE
                   WHEN PRICE_CATEGORY = 'SPECIAL' THEN SLS.EXT_SALES_AMOUNT
                   ELSE 0
                END)
                SP_SALES,
             SUM (
             ...
             SUM (
                CASE
                   WHEN PRICE_CATEGORY = 'CREDITS' THEN SLS.EXT_SALES_AMOUNT
                   ELSE 0
                END)
                CREDITS_SALES,
             SUM (
