...
/*
      killed at 50 - min
*/
CREATE TABLE AAA6863.PR_VICT2_CUST_DAY
AS
   SELECT DISTINCT
          sp_dtl.YEARMONTH,
          ...
          sp_dtl.IHF_INSERT,
          sp_dtl.ILF_INSERT INSERT_TIMESTAMP,
          sp_dtl.IHF_UPDATE,
          sp_dtl.ILF_UPDATE
   FROM (SELECT SP_HIST.*, --process date changed to include invoice processing date
         ...
         FROM (SELECT IHF.ACCOUNT_NUMBER,
               ...
               FROM DW_FEI.INVOICE_HEADER_FACT IHF,
                    ...
                    PRICE_MGMT.PR_LAST_RUN LAST_RUN
               ...
               WHERE     IHF.INVOICE_NUMBER_GK = ILF.INVOICE_NUMBER_GK
                     ...
                     AND ILF.INSERT_TIMESTAMP > LAST_RUN.MAX_ILF_TS) SP_HIST
              ...
              (SELECT COD.BASIS,
               ...
               WHERE     COD.OVERRIDE_TYPE = 'G'
                     ...
                     AND CUST.JOB_YN = 'Y'
                     AND NVL (COD.EXPIRE_DATE, SYSDATE) >= (SYSDATE - 3))
              GR_OVR_JOB
