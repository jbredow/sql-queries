...
--Edited to format raw VICT2 download by Corey Sykes - RPC Atlantic - 12/12/14

SELECT DISTINCT
             (sp_dtl.INVOICE_NUMBER_NK || '*' || sp_dtl.INVOICE_LINE_NUMBER) INVOICE,
             sp_dtl.WAREHOUSE_NUMBER SELLWHSE,
             sp_dtl.INVOICE_NUMBER_NK INVNO,
             sp_dtl.ORDER_CODE ORDCDE,
             sp_dtl.SHIP_TO_NAME SHIPTONAME,
             CASE
              WHEN sp_dtl.TYPE_OF_SALE = 'SHOWROOM' THEN 'Y'
              ELSE 'N'
              END SHOWROOM,
             sp_dtl.PROCESS_DATE,
             sp_dtl.CUSTOMER_NAME CUSTNAME,
             sp_dtl.CONTRACT_NUMBER CONTRACTNO,
             sp_dtl.CONTRACT_DESCRIPTION CONTRACTDESC,
             sp_dtl.PO_WAREHOUSE_NUMBER PO_WHSE,
             sp_dtl.PO_NUMBER CUST_PO,
             sp_dtl.ALT1_CODE ALT1_SP,
             sp_dtl.PRODUCT_NAME Description,
             sp_dtl.SHIPPED_QTY Shipped,
             sp_dtl.PACK_QTY Per,
             sp_dtl.UNIT_NET_PRICE_AMOUNT Net_Price,
             CASE 
              WHEN sp_dtl.UNIT_NET_PRICE_AMOUNT >= 0 THEN NULL
              ELSE ABS(sp_dtl.UNIT_NET_PRICE_AMOUNT)
              END XNETPRICE,
             sp_dtl.MATRIX_PRICE MATRIX,
             sp_dtl.LIST_PRICE BASE_PRICE,
             (sp_dtl.EXT_AVG_COGS_AMOUNT/sp_dtl.SHIPPED_QTY) AVG_COST,
             sp_dtl.UNIT_INV_COST INV_COST,
             CASE 
              WHEN sp_dtl.UNIT_INV_COST >= 0 THEN NULL
              ELSE ABS(sp_dtl.UNIT_INV_COST)
              END XNETCOST,
             sp_dtl.REPLACEMENT_COST REPCOST,
             sp_dtl.DISCOUNT_GROUP_NK DISCGROUP,
             sp_dtl.LIST_PRICE LIST,
             sp_dtl.PRICE_FORMULA FORMULA,
             sp_dtl.PRICE_CODE PRCODE,
             sp_dtl.BASIS_8 BASIS8,
             sp_dtl.LINEBUY_NK LINE,
             sp_dtl.MEMO MEMO,
             sp_dtl.CREDIT_MEMO_TYPE CM_CODE,
             sp_dtl.UPC_CODE UPC_CODE,
             sp_dtl.OML_ASSOC_INI WRTR,
             sp_dtl.CUSTOMER_NK CUSTNO
             ...
             IHF.YEARMONTH,
                  ...
                  LEFT OUTER JOIN (SELECT COD.BASIS,
                     ...
                     ON (    SP_HIST.PRODUCT_NK = PR_OVR.MASTER_PRODUCT
                         ...
                         AND NVL(SP_HIST.CONTRACT_NUMBER,'DEFAULT_MATCH')=NVL(PR_OVR.CONTRACT_ID,'DEFAULT_MATCH'))) sp_dtl
    --where sp_dtl.ORDER_ENTRY_DATE between nvl(sp_dtl.CCOR_CREATE,sp_dtl.ORDER_ENTRY_DATE)
    --and nvl(sp_dtl.CCOR_EXPIRE,sp_dtl.ORDER_ENTRY_DATE)
