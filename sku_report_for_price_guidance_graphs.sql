/*
		SKU Report for graph data
*/

SELECT DISTINCT
       sp_dtl.ACCOUNT_NUMBER BR_NO,
       sp_dtl.ACCOUNT_NAME BRANCH,
       sp_dtl.PRICE_COLUMN PC,
       PC_DEF.PC_SHORT_CAT PC_CAT,
       sp_dtl.DISCOUNT_GROUP_NK DG_NO,
       sp_dtl.DISCOUNT_GROUP_NAME,
       sp_dtl.ALT1_CODE,
       sp_dtl.PRODUCT_NAME,
       sp_dtl.TYPE_OF_SALE SALE_TYPE,
			 
			 -- clean-up price_cat
			 CASE
				WHEN  ( sp_dtl.PRICE_FORMULA LIKE ( 'L-0.___' ) )
				THEN 
					'MATRIX'
			 	WHEN sp_dtl.PRICE_CODE = 'N/A' 
					AND sp_dtl.PRICE_FORMULA = 'N/A' 
					AND sp_dtl.UNIT_NET_PRICE_AMOUNT = sp_dtl.MATRIX
				THEN
					'OVERRIDE'
				WHEN sp_dtl.PRICE_CODE = 'Q'
				THEN
					'MANUAL'
				
				WHEN sp_dtl.PRICE_CATEGORY = 'MATRIX'
					AND ( sp_dtl.PRICE_FORMULA NOT LIKE ( 'L-0.___' ) )
					AND ( sp_dtl.UNIT_NET_PRICE_AMOUNT = sp_dtl.MATRIX )
				THEN
					'CONTRACT'
				WHEN sp_dtl.PRICE_CATEGORY = 'MATRIX'
       ...
       ROUND ( sp_dtl.UNIT_NET_PRICE_AMOUNT
              / CASE
                  WHEN sp_dtl.LIST_PRICE > 0 THEN ( sp_dtl.LIST_PRICE )
                  ELSE 1
                END,
              3
       )
       ...
       sp_dtl.PRICE_FORMULA "FORM",
       sp_dtl.LIST_PRICE "LIST",
       sp_dtl.MATRIX_PRICE,
       sp_dtl.MATRIX,
       sp_dtl.SHIPPED_QTY SHPD,
       sp_dtl.MAIN_CUSTOMER_NK MAIN_NO,
       sp_dtl.CUSTOMER_NK CUST_NO,
       sp_dtl.CUSTOMER_NAME,
			 sp_dtl.ORDER_ENTRY_DATE,
       sp_dtl.INVOICE_NUMBER_NK INV_NO,
       sp_dtl.OML_ASSOC_NAME OML_ASSOC,
       sp_dtl.WRITER WRTR
  FROM     ( SELECT SP_HIST.*,
                    CASE
                      ...
                      THEN
                        CASE
                          WHEN SP_HIST.ORDER_ENTRY_DATE BETWEEN PR_OVR.INSERT_TIMESTAMP
                          ...
                          THEN
                            CASE
                              ...
                              THEN
                                'OVERRIDE'
                              WHEN SP_HIST.ORDER_ENTRY_DATE BETWEEN GR_OVR.INSERT_TIMESTAMP
                              ...
                              THEN
                                CASE
                                  WHEN REPLACE ( SP_HIST.PRICE_FORMULA,
                                         ...
                                         REPLACE ( NVL ( PR_OVR.FORMULA, GR_OVR.FORMULA ),
                                                  '0.',
                                                  '.'
                                         )
                                  THEN
               ...
               FROM           ( SELECT IHF.ACCOUNT_NUMBER,
                                       ...
                                       END
                                         AS STATUS,
                                       CASE
                                         WHEN ILF.EXT_SALES_AMOUNT <= 1500
                                         THEN
                                           'Y'
                                         ELSE
                                           'N'
                                 ...
                                 WHERE IHF.INVOICE_NUMBER_GK =
                                         ILF.INVOICE_NUMBER_GK
                                       -- AND ILF.PRODUCT_STATUS = 'SP'
                                       /*AND IHF.ACCOUNT_NUMBER IN ( '226',
                                                                                            '448',
                                                                                             '520',
                                                                                             '1020',
                                                                                             '1550',
                                       ...
                                       AND DECODE ( NVL ( cust.ar_gl_number, '9999' ),
                                            ...
                                            '9999', 0,
                                            1 ) <> 0
                                       AND NVL ( IHF.CONSIGN_TYPE, 'N/A' ) NOT IN
                                              'R'
                          ...
                          LEFT OUTER JOIN
                            DW_FEI.LINE_BUY_DIMENSION LB
                          ON SP_HIST.LINEBUY_NK = LB.LINEBUY_NK
                        LEFT OUTER JOIN
                          DW_FEI.MASTER_VENDOR_DIMENSION MV
         ...
         ON sp_dtl.ACCOUNT_NUMBER = ACCOUNT_NUMBER_NK
       LEFT OUTER JOIN
         AAA6863.PRICE_COLUMN_DEFINITION PC_DEF
       ON ( sp_dtl.PRICE_COLUMN = PC_DEF.PRICE_COLUMN )
 WHERE     ( SUBSTR ( SWD.REGION_NAME,
                     1,
                     3
            ) IN ('D10', 'D11', 'D12', 'D13', 'D14', 'D30', 'D31', 'D32') )
       /*AND ( sp_dtl.ALT1_CODE IN (  'DR18BPX','DR18BP12' ))*/
       AND ( sp_dtl.ALT1_CODE NOT LIKE 'SP-%' )
       AND ( sp_dtl.DISCOUNT_GROUP_NK IN ('0504', '0505') )
       AND ( sp_dtl.PRICE_CATEGORY <> 'CREDITS' )
       AND ( sp_dtl.LIST_PRICE > 0 )
       AND ( sp_dtl.UNIT_NET_PRICE_AMOUNT > 0 )
