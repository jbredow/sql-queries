SELECT DISTINCT
       ...
       END
         ST,
       SUM ( SLS.EXT_AVG_COGS_AMOUNT ) EX_SALES,
       SUM ( SLS.EXT_ACTUAL_COGS_AMOUNT ) EX_AC
  FROM   ( SELECT ILF.YEARMONTH,
       ...
       LEFT OUTER JOIN
         SALES_MART.SALES_WAREHOUSE_DIM SWD
			 ...
			 LEFT OUTER JOIN SALES_MART.TIME_PERIOD_DIMENSION TD
...
GROUP BY SLS.WAREHOUSE_NUMBER_NK, CASE
                                    WHEN SLS.STATUS = 'SP-' THEN 'SP-'
                                    WHEN SLS.STATUS = 'SP' THEN 'SP'
                                    ELSE 'OTHER'
                                  END;