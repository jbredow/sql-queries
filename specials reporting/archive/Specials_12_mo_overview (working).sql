SELECT DISTINCT --SPECIALS.DIVISION_NAME REGION,
			SPECIALS.YEARMONTH,
			--SPECIALS.REGION_NAME DIST,
			SPECIALS.ACCOUNT_NUMBER BR_NO,
			...
			SPECIALS.PRODUCT_STATUS ST,
			SUM ( SPECIALS.EXT_SALES_AMOUNT ) EXT_SALES,
			SUM ( SPECIALS.EXT_AVG_COGS_AMOUNT ) EXT_AC,
      SUM (
				CASE
					WHEN SUBSTR ( SPECIALS.ALT1_CODE, 1, 3 ) = 'SP-' THEN ( SPECIALS.EXT_SALES_AMOUNT )
					ELSE 0
				END
				) "SP- SALES",
			SUM (
				CASE
					WHEN SUBSTR ( SPECIALS.ALT1_CODE, 1, 3 ) = 'SP-' THEN ( SPECIALS.EXT_AVG_COGS_AMOUNT )
					ELSE 0
				END
				) "SP- AC",
			SUM (
				CASE
					WHEN SPECIALS."<1500" = 'Y' THEN ( SPECIALS.EXT_SALES_AMOUNT )
					ELSE 0
				END
				) "<$1500 SALES",
			SUM (
				CASE
					WHEN SPECIALS."<1500" = 'Y' THEN ( SPECIALS.EXT_AVG_COGS_AMOUNT )
					ELSE 0
				END
				) "<$1500 AC"
